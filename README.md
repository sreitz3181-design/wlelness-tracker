...
24. **Medications & Supplements, split by function:** setup (name, dose, Morning/Night/Anytime, with full Edit) lives on the **Weekly Planner** — these are standing entries, not tied to a specific week, so once added they just carry forward automatically. Daily "Mark taken" check-off lives on the **Nutrition** screen, grouped into Morning/Night/Anytime sections. Deliberately kept simple and isolated either way — no AI commentary on your regimen, never sent to journal feedback, weekly review, or any other AI feature. "Remove" soft-deletes (sets inactive) rather than destroying history.
...
30. **Planned meals are now tied to specific days.** Dinner's 7 slots are fixed to Monday-Sunday (labeled as such, not reorderable). Breakfast/Lunch/Snacks slots each get an optional "day" dropdown when you add one — leave it "Any day" if you don't want it tied to a specific day, or pick one so it can flow into the Nutrition screen automatically.
31. **Nutrition now defaults from the Weekly Planner.** On load, any meal category you haven't already chosen today gets prefilled from whatever's day-tagged for today in the Weekly Planner (leftovers resolve to the underlying dinner's recipe and its calories) — shown with a small "from plan" tag next to the label. Picking something different simply overrides it for today; the default never overwrites a choice you've already made.
32. **Medications split by function** — setup/editing on the Weekly Planner (standing entries, carry forward automatically), daily "taken" check-off on Nutrition, grouped Morning/Night/Anytime.
33. **Email reminders are live.** Two Vercel Cron jobs (`vercel.json`), each hitting a route that sends an email via Resend:
    - Daily, 8:30 AM Central → `/api/cron/daily-reminder`
    - Weekly, Friday 5:00 PM Central → `/api/cron/weekly-reminder`

    **Setup required:**
    - Sign up at resend.com, get an API key, add `RESEND_API_KEY` in Vercel
    - Add `REMINDER_EMAIL` (where reminders go) and `CRON_SECRET` (any random string — authenticates the cron requests so the routes can't be triggered by anyone who finds the URL)
    - Redeploy after adding env vars (same build-time rule as always)

    **Two honest caveats:**
    - The cron schedule is fixed in UTC (`vercel.json`'s `30 13 * * *` and `0 22 * * 5`), computed against Central *Daylight* Time. When Daylight Saving ends, both reminders will fire an hour earlier by the clock (7:30 AM / 4:00 PM) until the schedule is manually adjusted by an hour — a known limitation of fixed-UTC cron against a DST-observing timezone, not a bug.
