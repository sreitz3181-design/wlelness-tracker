...
// Upserts one or more fields on the daily_logs row for `date` (a
// YYYY-MM-DD string, defaulting to today), creating it on first write.
// `fields` is an object, e.g. { workout_type: 'strength' }.
export async function saveDayFields(userId, fields, date = todayISO()) {
...
export async function saveTodayFields(userId, fields) {
  return saveDayFields(userId, fields)
}
